...
|---|---|---|---|
| `AGENTICORG_WORKER_PROCESS` | process (Celery worker and beat only) | unset | `1` lets `core.tasks.async_runner.run_async` keep one event loop for the whole process, which is safe only where it is the only loop. `scripts/run_worker.py` and `scripts/run_beat.py` set it; the Celery `worker_process_init` signal also marks a prefork worker, but not `--pool=solo`, `threads` or gevent. Never set it on the API. |
| `AGENTICORG_DB_CROSS_LOOP_GUARD` | process | `warn` | What happens when the shared pool is used from a second event loop: `warn` logs `db_cross_loop_use` once per foreign loop and counts `agenticorg_db_cross_loop_checkouts_total`, `raise` fails the call with `CrossLoopConnectionError`, `off` disables the check. Warn in production: the call fails either way, and raising would turn latent pool problems into new 500s. A test run additionally counts the trips and fails above `cross_loop_baseline.txt`; `off` stops the counting and `AGENTICORG_CROSS_LOOP_BASELINE=<n>` replaces the number for one run. Both are deliberate maintainer overrides and both still exit 0 — a run with the guard off says so on its summary line. |
...
| `checkpoint_encryption_key_invalid` / `checkpoint_encryption_key_missing` | `AGENTICORG_VAULT_KEYRING` is malformed or empty, or no vault key is set outside a local or test runtime | fix the keyring (`id:secret,...`), then restart API and workers |
