...
Real POS/provider callbacks must be signed by the POS/provider integration. AgenticOrg computes `callback_verified` server-side; callers cannot set it in the JSON body.

Required headers:

| Header | Value |
| --- | --- |
| `X-AgenticOrg-POS-Timestamp` | Unix seconds or ISO timestamp generated by the POS/provider integration. |
| `X-AgenticOrg-POS-Signature` | `sha256=<hex-hmac>` over `<timestamp>.<raw request body>` using `OFFLINE_POS_WEBHOOK_SECRET`. |

The verification window is five minutes. Missing, stale, malformed, or mismatched signatures fail closed. When `OFFLINE_POS_WEBHOOK_SECRET` is configured, `payment_confirmed` and `receipt_available` confirmations are rejected unless the callback signature verifies. Non-payment statuses can still be reconciled safely as staff-review or blocker outcomes.
