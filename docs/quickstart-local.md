...
| Command | Runs |
|---|---|
| `make test` | `tests/unit`, `tests/security` and the contract suite `tests/connector_harness` in one run with coverage and the 55% floor, then `make test-integration` with its coverage added, so `coverage.xml` covers both |
