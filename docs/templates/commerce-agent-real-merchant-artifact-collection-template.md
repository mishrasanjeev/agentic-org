# Commerce Agent Real Merchant Artifact Collection Template

Status: template only
Date: 2026-05-26
Scope: repo-safe placeholder template for AgenticOrg dependency artifacts
before any future public commerce discovery proposal
Production changes made by this template: none
AgenticOrg public commerce discovery changed by this template: no
Grantex production Commerce V1 changed by this template: no
...
- A completed template is still not an AgenticOrg rollout approval.
- Synthetic merchant IDs, names, and payloads are never production candidates.
- AgenticOrg may reference only reviewed Grantex-controlled public metadata.
...
| --- | --- | --- |
| Approved public merchant ID | `<MERCHANT_ID_PENDING_APPROVAL>` | Must be human-approved in Grantex before any future allowlist or AgenticOrg public discovery proposal; never use a synthetic ID. |
...
- This template does not enable AgenticOrg public commerce discovery.
...
- This template does not add a direct provider path.
