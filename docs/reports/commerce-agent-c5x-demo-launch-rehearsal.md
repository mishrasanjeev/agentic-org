# Commerce Agent C5X Demo Launch Rehearsal

Status: docs-only launch rehearsal
Date: 2026-05-27
Scope: merchant education walkthrough for a simulated AgenticOrg read-only
discovery follow-up
...
`docs/examples/commerce-agent-c5w-demo-home-goods-store-packet.json` to show how
AgenticOrg would stay gated while a merchant publish flow is reviewed. The
packet is demo-only, is not a real merchant approval, and is never a production
allowlist candidate.

## Demo Launch Flow

1. Merchant completes the profile.
   - Merchant fills public-safe identity fields and discovery wording in the
     Grantex-controlled onboarding flow.
...
2. Public payload preview is reviewed.
   ...
   - AgenticOrg may only display reviewed Grantex-controlled metadata.
...
6. Rollout proposal is prepared.
   ...
   - Preparing a proposal does not enable AgenticOrg public commerce discovery.
...
9. Read-only discovery launch would be enabled only after separate approval.
   - Grantex production remains fail-closed until later approval exists.
...
- Launch is not automatic because public discovery affects production
  exposure, rollback, support posture, and AgenticOrg dependency state.
- Rollback requires an assigned owner, evidence retention, a smoke result, and
  independent AgenticOrg disable behavior.
...
AgenticOrg public commerce discovery, does not enable Commerce V1, does not
enable checkout or payment creation, does not enable live payments, does not
enable live Plural, and does not add provider credential access.
