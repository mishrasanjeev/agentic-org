...
| Integrations | Native registry and optional gateways, with tenant credentials and scopes. Registry entries are not active connections. The Connectors UI now projects scoped credential presence and recent/stale/failed/unknown health evidence from existing configuration metadata. | Homepage claims 250+ integrations; docs show install, credential, function execution, MCP and audit surfaces. | **P0, partially fixed:** add provider scope and contract verification, successful-sync lag, and measured error budgets; verify top 20 customer integrations end to end. The new readiness badge is not a certification of a working integration. |
...
| Models | Gemini/Claude/OpenAI support and primary/fallback in `core/llm/router.py`; local/test modes. The legacy completion path now has a bounded deadline and transient-only fallback. | EmaFusion docs describe task routing and model controls; homepage claims 100+ models and cost savings. | **P1, partially fixed:** extend deadline/fault policy across every model path, add provider-health-aware routing and per-tenant budget reservation, then run a measured eval/cost corpus. No cheaper or more accurate claim until matched tests. |
