...
AgenticOrg public commerce discovery changed by this checklist: no
Grantex production Commerce V1 changed by this checklist: no
...
This checklist defines what real human artifacts must exist before AgenticOrg
can prepare any later public commerce discovery proposal. It does not approve a
merchant, approve AgenticOrg public discovery, approve a Grantex production
allowlist entry, or authorize any runtime change.
...
- Real merchant artifact intake is required before any AgenticOrg public
  commerce discovery proposal.
...
- Do not use synthetic merchant IDs, names, or payloads for any production
  allowlist, AgenticOrg public discovery, or rollout proposal.

## Required Human Approval Artifacts

Every artifact below must be supplied by an authorized human reviewer before
the AgenticOrg dependency intake can be considered complete.

| Artifact | Required evidence | Repository-safe record |
| --- | --- | --- |
| Merchant owner approval | The merchant owner approves AgenticOrg referencing reviewed Grantex-controlled public read-only metadata for one named merchant. | Public-safe approval status and non-secret reference label only. |
| Legal/compliance approval | Legal/compliance approves AgenticOrg exposure of the reviewed public metadata and wording posture. | Public-safe approval status and non-secret reference label only. |
| Product wording approval | Product approves AgenticOrg wording and confirms no checkout, payment, live-provider, readiness, or certification claim is implied. | Public-safe approval status and non-secret reference label only. |
| Security approval | Security confirms AgenticOrg exposes no secrets, private merchant details, provider credentials, direct provider paths, or payment material. | Public-safe approval status and non-secret reference label only. |
| Ops/on-call/support approval | Ops confirms AgenticOrg support, monitoring, on-call, incident, and escalation posture. | Role or queue posture only; no private contacts. |
| Backup/RPO approval | Backup/RPO owner confirms AgenticOrg metadata exposure and rollback do not weaken recovery posture. | Public-safe approval status and non-secret reference label only. |
| AgenticOrg dependency approval | Dependency owner confirms Grantex approval and smoke evidence are prerequisites before AgenticOrg public discovery can be proposed. | Public-safe approval status and non-secret reference label only. |
| Rollback owner approval | A rollback owner accepts responsibility for hiding AgenticOrg commerce metadata again. | Owner role or non-secret reference label only. |
| Read-only smoke owner approval | A smoke owner accepts responsibility for future AgenticOrg GET-only evidence collection. | Owner role or non-secret reference label only. |
| Evidence retention owner approval | An evidence owner accepts responsibility for scrubbed AgenticOrg proof retention. | Owner role or non-secret reference label only. |

## Required Merchant Identity Fields

The intake packet must identify exactly one real Grantex-controlled merchant
using reviewed public-safe values. These values remain unapproved for
AgenticOrg rollout until a separate rollout proposal is reviewed.

| Field | Requirement | Repository handling |
| --- | --- | --- |
| Approved public merchant ID | Human-reviewed Grantex public identifier for one merchant. | Record only after approval; never use a synthetic ID. |
...
Before any future AgenticOrg public discovery proposal, reviewers must see the
exact public payload that would be exposed. The preview must contain only
approved Grantex-controlled public-safe values and must explicitly avoid
checkout, payment, live-provider, readiness, and certification claims.
...
- Direct provider credentials and direct provider calls are out of scope.
...
- Approval that AgenticOrg exposes only reviewed Grantex-controlled metadata.
...
- AgenticOrg remains gated until separate approval exists.
...
- AgenticOrg is asked to expose public commerce discovery before Grantex
  approval and separate AgenticOrg dependency approval exist.
...
- Allowlist value review using only a human-approved Grantex merchant ID.
...
- AgenticOrg dependency review confirming public commerce discovery remains
  gated until Grantex approval and separate AgenticOrg approval exist.

## Stop Conditions

Stop and do not prepare a future AgenticOrg rollout proposal if any condition
below is true:
...
- AgenticOrg public discovery is requested before Grantex named merchant
  approval, Grantex read-only smoke review, and separate AgenticOrg approval
  exist.
...
| Rollout proposal ready | All required artifacts are complete and reviewed, with no secrets, private data, or overclaims. | Prepare a separate proposal that still requires explicit approval before any config, allowlist, or AgenticOrg public discovery change. |
...
- This checklist does not approve a Grantex production allowlist value.
- This checklist does not enable AgenticOrg public commerce discovery.
- This checklist does not enable `AGENTICORG_COMMERCE_PUBLIC_DISCOVERY_ENABLED`.
...
- This checklist does not add direct provider handling to AgenticOrg.
