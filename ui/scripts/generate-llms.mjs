...
function summary(data) {
  const lines = [
    ...
    "- Governed cases are tenant-gated. Repository source checks a role's registered tools, an exact local case-purpose allowlist and a delegated grant before provider calls; this does not prove a hosted deployment enabled the flow.",
    "- Published Grantex Python SDK 0.5.1 does not enforce a token-level case purpose or per-case cap. A machine credential or MCP tool cannot perform a human-only case decision.",
    ...
    "",
    "### Can an agent approve a governed business case?",
    "",
    "No. An agent can investigate with a valid tenant role, local case purpose and delegated tool grant. Review and case decisions require a signed-in human. See " + REPO + "/blob/main/docs/governance/case-lifecycle.md for setup and current SDK limits.",
    "",
    "## More",
    "",
...
function full(data) {
  ...
  const appendix = [
    ...
    "- Do not describe source-only governed-case controls as deployed until the live revision is verified; distinguish the local case-purpose allowlist from token-level purpose and per-case cap enforcement.",
