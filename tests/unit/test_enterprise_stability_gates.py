...
def test_gate_allows_broad_exception_with_specific_safety_reason(tmp_path: Path) -> None:
    path = _write(
        tmp_path,
        "api/v1/example.py",
        "def route():\n"
        "    try:\n"
        "        risky()\n"
        "    # enterprise-gate: broad-except-ok reason=cleanup-only-primary-operation-complete\n"
        "    except Exception:\n"
        "        pass\n",
    )
...
def test_broad_exception_reason_rejects_generic_words(tmp_path: Path) -> None:
    path = _write(
        tmp_path,
        "api/v1/example.py",
        "def route():\n"
...
def test_broad_exception_baseline_reduced_by_auth_bridge_cdc_connector_slice() -> None:
    baseline = gates.load_baseline(gates.DEFAULT_BASELINE)
    broad_entries = baseline.get("allowed_findings", {}).get("broad_exception", [])

    assert len(broad_entries) < 348
...
def test_broad_exception_baseline_reduced_by_ai_data_slice() -> None:
    baseline = gates.load_baseline(gates.DEFAULT_BASELINE)
    broad_entries = baseline.get("allowed_findings", {}).get("broad_exception", [])

    assert len(broad_entries) < 237
...
def test_broad_exception_baseline_reduced_by_admin_business_slice() -> None:
    baseline = gates.load_baseline(gates.DEFAULT_BASELINE)
    broad_entries = baseline.get("allowed_findings", {}).get("broad_exception", [])

    assert len(broad_entries) < 163
    assert len(broad_entries) <= 133


def test_broad_exception_baseline_reduced_by_core_runtime_slice() -> None:
    baseline = gates.load_baseline(gates.DEFAULT_BASELINE)
    broad_entries = baseline.get("allowed_findings", {}).get("broad_exception", [])

    assert len(broad_entries) < 132
    assert len(broad_entries) <= 102


def test_broad_exception_baseline_reduced_by_service_layer_slice() -> None:
    baseline = gates.load_baseline(gates.DEFAULT_BASELINE)
    broad_entries = baseline.get("allowed_findings", {}).get("broad_exception", [])

    assert len(broad_entries) < 90
    assert len(broad_entries) <= 60


def test_broad_exception_baseline_reduced_by_near_final_slice() -> None:
    baseline = gates.load_baseline(gates.DEFAULT_BASELINE)
    broad_entries = baseline.get("allowed_findings", {}).get("broad_exception", [])

    assert len(broad_entries) < 55
...
def test_core_execution_target_routes_have_metadata() -> None:
    ...
    assert len(routes) == 79  # +1: DELETE /agents/{id}/amendments/{index}
    assert findings == []
    assert all(route.metadata_present for route in routes)
    assert all(route.scope for route in routes)
    assert all(route.rate_limit for route in routes)
