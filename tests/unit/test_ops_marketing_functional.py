...
class TestOpsFunctional:
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_001_vendor_onboarding_clean(
        ...
        steps = [
            {
                ...
                "action": "validate_vendor_documents",
            },
            {
                "id": "sanctions_check",
                "type": "agent",
            ...
            },
            {
                ...
                "true_path": "create_erp_record",
                "false_path": "block_vendor",
            ...
            },
            {
                ...
                "type": "agent",
                "agent": "vendor_manager",
                "action": "create_oracle_fusion_vendor",
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_002_vendor_onboarding_sanctions_hit(
        self, workflow_engine, mock_state_store, base_state
    ):
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_004_l2_ticket_enrichment(
        self, workflow_engine, mock_state_store, base_state
    ):
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_005_contract_renewal_alert(
        self, workflow_engine, mock_state_store, base_state
    ):
        ...
        steps = [
            {
                ...
                "true_path": "send_alerts",
                "false_path": "no_action",
            },
            {
                "id": "send_alerts",
                "type": "parallel",
                "steps": ["slack_alert", "email_alert"],
                "wait_for": "all",
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_006_sla_breach_detection(
        self, workflow_engine, mock_state_store, base_state
    ):
        ...
        steps = [
            {
                ...
                "true_path": "compute_penalty",
                "false_path": "no_breach",
            },
            {
                "id": "compute_penalty",
                "type": "agent",
                "agent": "contract_intelligence",
                "action": "compute_sla_penalty",
        ...
        penalty = (
            payload["daily_penalty_rate"] * payload["contract_value"] * payload["days_overdue"]
        )
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_007_regulatory_filing_prep(
        self, workflow_engine, mock_state_store, base_state
    ):
        ...
        steps = [
            ...
            },
            {
                ...
                "type": "agent",
                "agent": "compliance_guard",
                "action": "prepare_regulatory_filing",
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_008_it_access_provisioning(
        self, workflow_engine, mock_state_store, base_state
    ):
        ...
        steps_sensitive = [
            ...
            },
            {
                "id": "hitl_approval",
                "type": "human_in_loop",
                "assignee_role": "it_security_lead",
                "timeout_hours": 4,
        ...
        assert (
            result_sensitive["step_results"]["hitl_approval"]["output"]["assignee_role"]
            == "it_security_lead"
        )
        ...
        steps_standard = [
            {
                ...
                "condition": "access_level == sensitive",
                "true_path": None,
                "false_path": "auto_provision",
            },
            {
                "id": "auto_provision",
                "type": "agent",
                "agent": "it_operations",
                "action": "provision_standard_access",
                "depends_on": ["classify_access"],
            },
        ]
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_009_it_incident_response_p1(
        self, workflow_engine, mock_state_store, base_state
    ):
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_ops_010_sar_draft_preparation(
        self, workflow_engine, mock_state_store, base_state
    ):
...
class TestMarketingFunctional:
    ...
    async def test_ft_mkt_001_lead_scoring(self, workflow_engine, mock_state_store, base_state):
        ...
        steps = [
            ...
            },
            {
                ...
                "true_path": "assign_to_sales",
                "false_path": "add_to_nurture",
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_mkt_002_campaign_budget_reallocation_hitl(
        self, workflow_engine, mock_state_store, base_state
    ):
        ...
        steps = [
            ...
            {
                "id": "check_realloc_amount",
                "type": "condition",
                "condition": "reallocation_amount > 50000",
                "true_path": "hitl_approval",
                "false_path": "auto_reallocate",
                "depends_on": ["analyze_performance"],
            },
            {
                "id": "hitl_approval",
                "type": "human_in_loop",
                "assignee_role": "marketing_head",
                "timeout_hours": 8,
        ...
        assert (
            evaluate_condition("reallocation_amount > 50000", {"reallocation_amount": 75000})
            is True
        )
    ...
    # -----------------------------------------------------------------------
    @pytest.mark.asyncio
    async def test_ft_mkt_003_brand_crisis_detection(
        self, workflow_engine, mock_state_store, base_state
    ):
        ...
        assert (
            evaluate_condition("neg_mentions_per_hour >= 100", {"neg_mentions_per_hour": 120})
            is True
        )
