...
(``python -I -B``: no environment variables, user site or working directory on
the import path), with a minimal environment and a fresh empty working
...
async def _run_worker(argv: list[str], payload: bytes, *, timeout_s: float, max_output_bytes: int) -> _Run:
    run = _Run()
    with tempfile.TemporaryDirectory(prefix="agenticorg-extract-") as workdir:
        try:
...
async def extract(
    ...
    *,
    ...
    argv = [sys.executable, "-I", "-B", str(worker_path), "--cpu-seconds", str(max(1, int(timeout_s) + 1))]
    ...
    if run.returncode != 0:
        ...
        if isinstance(response, dict) and response.get("ok") is False and response.get("reason") in WORKER_REASONS:
            reason = ExtractionFailure(response["reason"])
    ...
    try:
        ...
        return _failed(kind, source_hash, ExtractionFailure.OUTPUT_INVALID, started, detail=str(exc))
...
async def probe_isolation(
    *, timeout_s: float = DEFAULT_TIMEOUT_SECONDS, worker_path: Path = WORKER_PATH
    ...
    """
    argv = [sys.executable, "-I", "-B", str(worker_path), "--probe"]
    run = await _run_worker(argv, b"", timeout_s=timeout_s, max_output_bytes=DEFAULT_MAX_OUTPUT_BYTES)
    if run.returncode != 0:
        raise RuntimeError(f"sandbox probe did not complete (returncode={run.returncode}, timed_out={run.timed_out})")
